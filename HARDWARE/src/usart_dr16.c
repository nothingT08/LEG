...
void RC_Init(uint8_t *rx1_buf, uint8_t *rx2_buf, uint16_t dma_buf_num)//数组0 1   缓存长度
{
    ...
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE);// | RCC_AHB1Periph_DMA2
    ...
    GPIO_PinAFConfig(GPIOA, GPIO_PinSource3, GPIO_AF_USART2); //PC11  usart4 rx
    /* -------------- Configure GPIO ---------------------------------------*/
    {
        ...
        USART_ITConfig(USART2, USART_IT_IDLE, ENABLE);//总线空闲标志
...
void RC_restart(uint16_t dma_buf_num)
{
    ...
    USART_ClearFlag(USART2, USART_FLAG_IDLE);		//总线空闲标志
